...
Pipeline reproducible para **descargar índices**, **generar proyecciones de corto plazo** y **publicar insumos reutilizables** para otros análisis (investigación, reportes, tableros o modelos downstream).

## Qué resuelve este repositorio
1. Extrae histórico diario de índices (Stooq con fallback Yahoo).
2. Normaliza y deduplica la serie histórica.
3. Proyecta hasta 30 días hábiles con ARIMA sobre retornos log.
4. Exporta artefactos de datos (`CSV`, `Parquet`, `JSON`) para reutilización.
5. (Opcional) sincroniza resultados a Google Sheets para consumo por dashboard.
...
- Pestañas: `indices` y `proyecciones_30d`
